...
fun Any.get(mPath: String, tag: Any? = null, defaultFormat: Boolean = false) = NetKit.getkt(ApiBase.HOST
        ?: "", mPath, tag
        ?: this)

fun Any.get(url: String, mPath: String, tag: Any? = null, defaultFormat: Boolean = false) = NetKit.getkt(url, mPath, tag
        ?: this)

fun Any.post(mPath: String, tag: Any? = null) = NetKit.postkt(ApiBase.HOST ?: "", mPath, tag
        ?: this)
...
class NetKitGetUrl(val url: String, val mPath: String, val tag: Any? = null) {
    ...
    fun <T : Any> FileConvert(kcls: KClass<T>, folderPath: String, namePath: String) = convert(FileConvert(folderPath, namePath))
}
